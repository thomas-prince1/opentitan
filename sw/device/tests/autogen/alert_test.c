...
static void init_peripherals(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_ENGLISHBREAKFAST_SRAM_CTRL_RET_AON_REGS_BASE_ADDR);
...
static void trigger_alert_test(void) {
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopEnglishbreakfastAlertIdHmacFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopEnglishbreakfastAlertIdSramCtrlRetAonFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
